#!/bin/bash

echo "DISABLE STEAM CLOUD FOR BEST EXPERIENCE"

### Variables
DESKTOPFILE=${DESKTOPFILE:=~/.local/share/applications/Undertale.desktop}
SAVE_LOCATION=${SAVE_LOCATION:=~/.local/share/UNDERTALESAVESWITCHER}
APPDATAFOLDER=${APPDATAFOLDER:=~/.config/UNDERTALE}
###

echo "DESKTOP FILE: $DESKTOPFILE"
echo "SAVE LOCATION: $SAVE_LOCATION"
echo "CONFIG FOLDER: $APPDATAFOLDER"

mkdir -p $SAVE_LOCATION
if [ ! -d "$SAVE_LOCATION/DEFAULT" ]; then
  echo No Default Save File
  mkdir $SAVE_LOCATION/DEFAULT
  cp $APPDATAFOLDER/* $SAVE_LOCATION/DEFAULT
  rm -ir $APPDATAFOLDER/
fi
echo "AVALIABLE SAVES (TO MAKE NEW ONES JUST TYPE A UNLISTED NAME)"
ls $SAVE_LOCATION
echo "SAVENAME:"
read SAVENAME

unlink $APPDATAFOLDER
mkdir -p $SAVE_LOCATION/$SAVENAME
touch $SAVE_LOCATION/$SAVENAME/NOTEMPTY
ln -s $SAVE_LOCATION/$SAVENAME $APPDATAFOLDER

dex $DESKTOPFILE
